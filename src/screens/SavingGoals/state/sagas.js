...
const withdrawGoalSaga = function*() {
  yield takeEvery(`${WITHDRAW_GOAL_URL}_SUBMIT`, function*({ payload }) {
    yield put(requestApi(`${WITHDRAW_GOAL_URL}`, { data: payload }));
  });
};

const withdrawGoalSucceedSaga = function*() {
  yield takeEvery(`${WITHDRAW_GOAL_URL}_SUCCEED`, function*({ payload }) {
    yield put(updateGoalsData(payload));
  });
};
